...
from .kids_rawdata import KissRawData
...
flag = xpos <= 0.0
interp, dumm = kids_itp_flag(
    intermed, x=np.abs(xpos), kind="slinear", flag=flag
)  # did not see diff between slinear and linear
...
finter_arr = interp1d(np.abs(xpos[flag]), intermed_arr[:, flag], kind="slinear", axis=1, fill_value="extrapolate")
...
specnew = np.fft.fft(-1.0 * np.array(np.roll(intermednew - internewbase, -nzero), dtype=np.complex))[
    0 : len(xposnew) // 2
].real
specnew_arr = np.fft.fft(
    -1.0 * np.array(np.roll(intermednew_arr - internewbase_arr, -nzero, axis=1), dtype=np.complex), axis=1
)[:, 0 : len(xposnew) // 2].real
